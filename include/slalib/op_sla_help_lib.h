	...
	if (ctinfo == IP_CT_NEW) {
		...
		if (tp && iph && iph->protocol == IPPROTO_TCP) {
			...
			if (total_retrans > ct->op_tcp_last_total_retrans) {
				ct->op_tcp_continue_retrans++;
				for (i = 1; i <= ct->op_tcp_continue_retrans;
				     i++)
					calc_rtt_by_dev_index(ifaceindex,
							      TCP_RETRAN_RTT);
			...
			if (ct->op_tcp_continue_retrans >= 3 &&
				...
				if (!sla_app_switch_enable) {
					...
					if (op_sla_enable &&
					    try_to_fast_reset(top_app_type)) {
						ct->mark = 0x0;
						icsk = inet_csk(sk);
						icsk->icsk_rto = TCP_RTO_MIN;
						sk->op_sla_mark |=
...
static int sla_mark_skb(struct sk_buff *skb, const struct nf_hook_state *state)
{
	...
	if (ret == SLA_SKB_MARKED) {
		if (!sla_app_switch_enable) {
			op_sla_send_to_user(SLA_NOTIFY_SWITCH_APP_NETWORK, NULL,
					    0);
			return NF_ACCEPT;
		}
		if (op_sla_enable) {
...
static unsigned int op_sla_pkt_monitor(void *priv, struct sk_buff *skb,
				       const struct nf_hook_state *state)
{
	struct iphdr *iph = NULL;
...
static int op_sla_iface_down(struct nlmsghdr *nlh)
{
	...
	if (index != -1) {
		sla_write_lock();
		memset(&op_sla_info[index], 0x0, sizeof(struct op_dev_info));
		sla_write_unlock();
	}
	return 0;
}
