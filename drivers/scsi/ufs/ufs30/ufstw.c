...
<<<<<<< Updated upstream:drivers/scsi/ufs/ufs30/ufstw.c
static int ufstw_read_lu_attr(struct ufstw_lu *tw, u8 idn, u32 *attr_val)
{
	struct ufs_hba *hba = tw->ufsf->hba;
	int err;
...
>>>>>>> Stashed changes:drivers/scsi/ufs/ufstw.c
}
					 ...
					 size_t count)
{
	...
	if (val) {
		if (ufstw_set_lu_flag(tw, QUERY_FLAG_IDN_WB_EN, &tw->tw_enable))
			ret = -ENODEV;
	} else {
		if (ufstw_clear_lu_flag(tw, QUERY_FLAG_IDN_WB_EN, &tw->tw_enable))
			ret = -ENODEV;
	}
	pm_runtime_put_sync(tw->ufsf->hba->dev);
