...
int switch_to_cell(int cell_quality_good, int game_rtt, int gamelostcount,
		   int game_switch_interval, int game_type);
int switch_to_wifi(int wlan_bad, int game_rtt, int gamelostcount,
		   int game_switch_interval, int game_type);
void reset_sla_game_app_rx_error(int game_type);
void reset_sla_game_app_rtt(int game_type);
void record_sla_game_cell_state(int game_type, int game_switch_interval,
				int time_now);
void record_sla_game_wifi_state(int game_type, int game_switch_interval,
				int time_now);
...
int is_detect_game_lost(int game_lost_count, int game_lost_count_threshold,
			int game_time_interval);
int is_support_detect_game_tx(int game_type, int special_rx_pkt_last_timestamp);
void get_rx_pkt_threshold(int game_type, int time_now,
			  int special_rx_pkt_last_timestamp, int *rtt_callback);
int data_stall_detect(int lastspecialrxtiming, int specialrxthreshold,
		      int datastalltimer, int datastallthreshold);
...
int get_rx_interval_error(int game_category, int time_now,
			  int rx_pkt_timestamp);
int is_need_check_game_rtt(int game_detect_status, int game_timestamp,
			   int skb_len);
...
int update_sla_tcp_info(unsigned int tcp_rtt, unsigned int total_retrans,
			unsigned int mark);
int is_tcp_unreachable(int ifaceindex, unsigned int tcp_rtt,
...
void update_sla_icmp_info(unsigned int dst_addr, unsigned int wifiip,
			  unsigned int cellip);
