...
// Declare missing functions if not declared in headers
int find_iface_index_by_mark(unsigned int mark);
void calc_rtt_by_dev_index(int index, int rtt);
...
static int *make_game_vals(struct nf_conn *ct, int game_rtt, int game_type)
{
...
static void game_app_switch_network(struct nf_conn *ct, struct sk_buff *skb)
{
...
#else
	int cell_quality_good =
		(op_sla_info[CELLULAR_INDEX].cur_score >= CELL_SCORE_BAD) ? 1 :
									    0;
	int wlan_bad = 0;
#endif
	...
	if (op_sla_debug) {
		...
		pr_info("[op_sla] %s: interval_for_switch_time:%d gamelostcount:%d\n",
			__func__,
			time_now - op_sla_game_app_list
					   .repeat_switch_time[game_type],
			gamelostcount);
	}
	...
	if (switch_to_cell(cell_quality_good, game_rtt, gamelostcount,
			   game_switch_interval, game_type) ||
	    fw_set_game_mark == 1) {
#else
	if (fw_set_game_mark == 1) {
		...
		init_rtt_queue_info();
#ifdef CONFIG_SLA_ALGO
		record_sla_game_cell_state(game_type, game_switch_interval,
					   time_now);
#endif
		...
		op_sla_send_to_user(SLA_SWITCH_GAME_NETWORK,
				    (char *)game_bp_info, sizeof(game_bp_info));
		return;
	}

#ifdef CONFIG_SLA_ALGO
	if (switch_to_wifi(wlan_bad, game_rtt, gamelostcount,
			   game_switch_interval, game_type) ||
	    fw_set_game_mark == 0) {
#else
	if (fw_set_game_mark == 0) {
		...
		init_rtt_queue_info();
#ifdef CONFIG_SLA_ALGO
		record_sla_game_wifi_state(game_type, game_switch_interval,
					   time_now);
#endif
		memset(game_bp_info, 0x0, sizeof(game_bp_info));
		game_bp_info[0] = game_type;
...
#ifdef CONFIG_SLA_ALGO
			get_game_interval(game_type, game_interval);
#else
			game_interval;
#endif
				  ...
				  enum ip_conntrack_info ctinfo)
{
	...
	if (is_support_detect_game_tx(game_type,
		...
		get_rx_pkt_threshold(game_type, time_now,
				     special_rx_pkt_last_timestamp,
				     rtt_callback);
		...
		if (data_stall_detect(lastspecialrxtiming, specialrxthreshold,
	...
	if (game_type) {
		if (datastalltimer >= datastallthreshold) {
			...
			rx_interval_error_estimator(game_type, datastalltimer);
				   ...
				   enum ip_conntrack_info ctinfo)
{
	...
	if (op_sla_debug) {
		...
		pr_info("[op_sla] %s: src port:%d ct state:%d up count:%d game_status:%d\n",
			__func__, ntohs(udp_hdr(skb)->source),
			XT_STATE_BIT(ctinfo), ct->op_game_up_count,
	...
	if (ct->op_game_up_count == 0) {
		...
		if (op_sla_debug) {
			...
			pr_info("[op_sla] %s: down count:%d same count:%d same_count_max:%d\n",
				__func__, ct->op_game_down_count,
			     ...
			     enum ip_conntrack_info ctinfo)
{
	...
	if (!game_start_state && (ct_mark & GAME_UNSPEC_MARK))
	...
	if (iph &&
		...
		if (ct_mark && skb->mark && ct_mark != skb->mark) {
			if (op_sla_debug) {
...
static void rtt_game_check(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	if (op_sla_debug) {
		if (iph && iph->protocol == IPPROTO_UDP &&
			...
			pr_info("[op_sla] %s: skb dev:%s game_status:%d\n",
				__func__, skb->dev->name, game_detect_status);
	...
	if (!iph || iph->protocol != IPPROTO_UDP)
		return;
#ifdef CONFIG_SLA_ALGO
	if (is_need_check_game_rtt(game_detect_status, game_timestamp,
				   skb_len)) {
...
static void rx_interval_error_check(struct nf_conn *ct, struct sk_buff *skb)
{
	...
	if (game_type) {
		...
		if (rx_pkt_timestamp) {
#ifdef CONFIG_SLA_ALGO
			special_rx_interval_error = get_rx_interval_error(
				game_category, time_now, rx_pkt_timestamp);
#endif
			sla_game_rx_error_write_lock();
...
static void parser_dns(struct sk_buff *skb)
{
	...
	unsigned int wifiip;
	unsigned int cellip;
	u8 buffer[sizeof(struct udphdr) + sizeof(struct dnshdr) + MAX_QUERY_LEN];
	struct iphdr *iph = ip_hdr(skb);
	struct udphdr *uh;
	struct dnshdr *dnsh;
	int packet_len = skb->len - IP_HDR_LEN;
	int query_len =
		packet_len - sizeof(struct udphdr) - sizeof(struct dnshdr);
	...
	/* Get UDP header */
	uh = skb_header_pointer(skb, IP_HDR_LEN,
				MIN(packet_len, sizeof(buffer)), buffer);
	if (!uh)
		return;
	...
	if (is_response(dnsh)) {
		...
		dst_addr = iph->daddr;
		wifiip = get_default_ipaddr_by_devname(
			op_sla_info[WLAN_INDEX].dev_name);
		cellip = get_default_ipaddr_by_devname(
			op_sla_info[CELLULAR_INDEX].dev_name);
#ifdef CONFIG_SLA_ALGO
...
static void parser_dns_query(struct sk_buff *skb, unsigned long time_now)
{
	...
	src_addr = iph->saddr;
	wifiip =
		get_default_ipaddr_by_devname(op_sla_info[WLAN_INDEX].dev_name);
	cellip = get_default_ipaddr_by_devname(
		op_sla_info[CELLULAR_INDEX].dev_name);
	if (src_addr == wifiip) {
#ifdef CONFIG_SLA_ALGO
...
static void notify_fw_network_switch(void)
{
	...
	if (!last_notify_fw_network_switch ||
	    (time_now - last_notify_fw_network_switch) >= 2000) {
		last_notify_fw_network_switch = time_now;
		op_sla_send_to_user(SLA_SWITCH_APP_NETWORK, NULL, 0);
	}
}

static unsigned int op_sla_rx_calc(void *priv, struct sk_buff *skb,
				   const struct nf_hook_state *state)
{
	struct nf_conn *ct = NULL;
	enum ip_conntrack_info ctinfo;
			      ...
			      const struct nf_hook_state *state)
{
	...
	if (op_sla_debug)
		pr_info("[op_sla] %s: len:%d mark:%x port:%d\n", __func__,
			skb->len, skb->mark, ntohs(tcp_hdr(skb)->source));
		...
		if (sk) {
			...
			if (ret == SLA_SKB_MARKED) {
				if (op_sla_enable && sla_app_switch_enable) {
					if ((skb->mark & MARK_MASK) ==
					    CELLULAR_MARK)
						notify_fw_network_switch();
					return sla_skb_reroute(skb, state);
				}
				if (!sla_app_switch_enable &&
				    (skb->mark & MARK_MASK) == CELLULAR_MARK)
					op_sla_send_to_user(
						SLA_NOTIFY_SWITCH_APP_NETWORK,
						NULL, 0);
				return NF_ACCEPT;
			...
			if (retran_mark) {
				...
				if (op_sla_debug)
					pr_info("[op_sla] %s: index: %x , dr: %d , dqc: %d, dfqt: %lu, time: %d\n",
						__func__, WLAN_INDEX,
						op_sla_info[WLAN_INDEX]
							.dns_refuse,
						op_sla_info[WLAN_INDEX]
							.dns_query_counts,
						op_sla_info[WLAN_INDEX]
							.dns_first_query_time,
						time_now);

				if (op_sla_debug)
					pr_info("[op_sla] %s: index: %x , dr: %d , dqc: %d, dfqt: %lu, time: %d\n",
						__func__, CELLULAR_INDEX,
						op_sla_info[CELLULAR_INDEX]
							.dns_refuse,
						op_sla_info[CELLULAR_INDEX]
							.dns_query_counts,
						op_sla_info[CELLULAR_INDEX]
		...
		if (!sk) {
			if (op_sla_debug) {
				pr_info("[op_sla] %s: len:%d port:%d ct->mark:%x skb->mark:%x\n",
					__func__, skb->len,
					ntohs(tcp_hdr(skb)->source), ct->mark,
					skb->mark);
			}
			if (op_sla_enable && sla_app_switch_enable)
				return sla_skb_reroute(skb, state);
...
#ifdef CONFIG_SLA_ALGO
			ifaceindex = update_sla_tcp_info(tcp_rtt, total_retrans,
							 sla_mark);
#endif

			int op_sla_function(struct sk_buff * skb)
			{
				unsigned long time_now =
					ktime_get_ns() /
					1000000; // Fix for warnings
				u32 mark = sk->op_sla_mark &
					   MARK_MASK; // Fix for warnings
				...
				if (total_retrans >
					...
					ct->op_tcp_continue_retrans++;
					for (i = 1;
					     i <= ct->op_tcp_continue_retrans;
					     i++)
						calc_rtt_by_dev_index(
							ifaceindex,
							TCP_RETRAN_RTT);
					ct->op_tcp_last_total_retrans =
						total_retrans;
				...
				if (ct->op_tcp_continue_retrans >= 3 &&
				    cell_quality_good) {
					if (!sla_app_switch_enable) {
						op_sla_send_to_user(
							SLA_NOTIFY_SWITCH_APP_NETWORK,
				...
				if (skb->mark == CELLULAR_MARK &&
				    sla_app_switch_enable && op_sla_enable)
					notify_fw_network_switch();
			}
		}
		if (op_sla_debug) {
			pr_info("[op_sla] %s: len:%d port:%d ct->mark:%x skb->mark:%x\n",
				__func__, skb->len, ntohs(tcp_hdr(skb)->source),
				ct->mark, skb->mark);
		}
		if (op_sla_enable && sla_app_switch_enable)
			return sla_skb_reroute(skb, state);
	...
	static int handle_game_app_skb(struct nf_conn * ct,
				       struct sk_buff * skb,
				       enum ip_conntrack_info ctinfo,
				       const struct nf_hook_state *state)
	{
	...
	static int sla_mark_skb(struct sk_buff * skb,
				const struct nf_hook_state *state)
	{
		struct nf_conn *ct = NULL;
		int ret = SLA_SKB_CONTINUE;
		...
		if (ret == SLA_SKB_MARKED) {
			if (!sla_app_switch_enable) {
				op_sla_send_to_user(
					SLA_NOTIFY_SWITCH_APP_NETWORK, NULL, 0);
				return NF_ACCEPT;
			}
			if (op_sla_enable) {
				notify_fw_network_switch();
	...
	static unsigned int op_sla(void *priv, struct sk_buff *skb,
				   const struct nf_hook_state *state)
	{
		int ret = NF_ACCEPT;
		struct nf_conn *ct = NULL;
	...
	static unsigned int op_sla_pkt_monitor(
	...
	static int op_sla_set_game_app_uid(struct nlmsghdr * nlh)
	{
		...
		memset(&op_sla_game_app_list, 0x0,
		       sizeof(struct op_game_app_info));
		init_rtt_queue_info();
	...
	static int op_sla_set_top_app_uid(struct nlmsghdr * nlh)
	{
		...
		if (top_app_list.count > 0 &&
		    top_app_list.count < TOP_APP_NUM) {
			for (i = 0; i < top_app_list.count; i++) {
	...
	static int op_sla_set_netlink_valid(struct nlmsghdr * nlh)
	{
		...
		if (op_sla_debug)
			pr_info("[op_sla] %s: wlan valid:%d cell valid:%d\n",
				__func__, op_sla_info[WLAN_INDEX].netlink_valid,
				op_sla_info[CELLULAR_INDEX].netlink_valid);
		return 0;
	}
	...
	{
		op_sla_rtt_detect =
			(nlh->nlmsg_type == SLA_ENABLE_GAME_RTT) ? 1 : 0;
		if (op_sla_debug)
			pr_info("[op_sla] %s: set game rtt detect:%d\n",
				__func__, op_sla_rtt_detect);
		return 0;
	}
	...
	static int op_sla_set_game_start_state(struct nlmsghdr * nlh)
	{
		...
		if (op_sla_debug) {
			pr_info("[op_sla] %s: set game_start_state = %d\n",
				__func__, game_start_state);
		}
		return 0;
	}

	static int sla_netlink_rcv_msg(struct sk_buff * skb,
				       struct nlmsghdr * nlh,
	...
	static int op_sla_netlink_init(void)
	{
		...
		op_sla_sock =
			netlink_kernel_create(&init_net, NETLINK_OP_SLA, &cfg);
	...
	static int __init op_sla_init(void)
	{
		...
		ret |= nf_register_net_hooks(&init_net, op_sla_ops,
					     ARRAY_SIZE(op_sla_ops));
		if (ret < 0) {
	...
	static void __exit op_sla_deinit(void)
	{
		...
		op_sla_netlink_exit();
		nf_unregister_net_hooks(&init_net, op_sla_ops,
					ARRAY_SIZE(op_sla_ops));
	}
